...
local function HandlePriceChance()
    ...
    if Config.Crypto.History[coin][4] then
        ...
        Config.Crypto.History[coin][#Config.Crypto.History[coin] + 1] = { PreviousWorth = prevValue, NewWorth = currentValue }
    end
    ...
    if crypto ~= nil then
        if Config.Crypto.Worth[crypto] ~= nil then
            ...
            if NewWorth ~= nil then
                ...
                TriggerClientEvent('QBCore:Notify', src, 'You have changed the value of ' .. Config.Crypto.Labels[crypto] .. ' from: $' .. Config.Crypto.Worth[crypto] .. ' to: $' .. NewWorth .. ' (' .. ChangeLabel .. ' ' .. PercentageChange .. '%)')
                ...
                TriggerClientEvent('QBCore:Notify', src, Lang:t('text.you_have_not_given_a_new_value', { crypto = Config.Crypto.Worth[crypto] }))
            end
        else
...
QBCore.Commands.Add('checkcryptoworth', '', {}, false, function(source)
    local src = source
    TriggerClientEvent('QBCore:Notify', src, Lang:t('text.the_qbit_has_a_value_of', { crypto = Config.Crypto.Worth['qbit'] }))
end)
    ...
    CreateThread(function()
        ...
        while (true) do
            ...
            if type(get_coin_price) == 'number' then
                ...
                MySQL.update('UPDATE crypto set worth = ? where crypto = ?', { get_coin_price, 'qbit' }, function(affectedRows)
